...
{
	const int length = 15;
	char* randomString = (char*)malloc((length + 1) * sizeof(char)); // выделяем память под строку
    int i;
    for (i = 0; i < length; i++) {
        randomString[i] = 'a' + rand() % 26;  // генерация случайной буквы от 'a' до 'z'
    }
    randomString[length] = '\0';  // устанавливаем символ конца строки
    return randomString;
}
...
void *serve_client(void *arg)
{
	...
	random_message = randomiser();
	writen(socket, random_message, 64);
	/*while((rc = reads(socket, s, MAXLINE)) > 0) 
	{
		
		if(writen(socket, random_message, rc) == -1) break;
	}*/
	Close(socket);
